...
#
# hiresdata = pd.read_csv(results.fname)
data = pd.read_csv('TestData/T001246_4pl_data.csv')
# harpsdata = pd.read_csv('HARPSN_2/TOI-1246-tng_harpn-0028-srv-rvs-sais.dat',delimiter='      ',header=None)
# harpsdata = harpsdata[[0,1,2]]
# harpsdata.columns = ['time', 'mnvel', 'errvel']
# harpsdata['tel'] = 'harps-n'
# simdata = pd.read_csv('sim_keck_rvs.csv')
# simdata['tel'] = 'j'

# data = pd.concat([hiresdata, harpsdata, simdata])
...
# planet_letters = {1: 'd', 2: 'b', 3: 'c', 4:'f'}
...
# anybasis_params['gamma_wiyn'] = radvel.Parameter(value=0.5, vary=True, linear=True)
# anybasis_params['jit_wiyn'] = radvel.Parameter(value=0.5)
...
priors = [radvel.prior.PositiveKPrior(nplanets), radvel.prior.Gaussian('per3', 18.652357, 0.00665), radvel.prior.Gaussian('per1', 4.307412,  0.001355), radvel.prior.Gaussian('per2', 5.903194,  0.000474),  radvel.prior.Gaussian('per4',  37.919841, 0.001333),radvel.prior.HardBounds('per5', 40,300),radvel.prior.HardBounds('jit_j', -20.0, 20.0)]
 #, radvel.prior.EccentricityPrior([4],upperlims=[0.377])] radvel.prior.Gaussian('per4', 37.919841, 0.001333),
